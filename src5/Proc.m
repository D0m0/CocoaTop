...
+ (NSString *)simplifyPathName:(NSString *)path
{
	...
	for (int i = 0; i < target.count; i++) {
		NSString *key = [target objectAtIndex:i], *val = [source objectAtIndex:i];
		if (!key.length)
			continue;
	...
	if (self = [super init]) {
		@autoreleasepool {
			...
			if (proc_pidpath(self.pid, buffer, sizeof(buffer)))
				...
				self.executable = [args objectAtIndex:0];
			self.args = @"";
			for (int i = 1; i < args.count; i++)
				self.args = [self.args stringByAppendingFormat:@" %@", [args objectAtIndex:i]];
			...
			if (self.app) {
				NSString *ident = [self.app objectForKey:@"CFBundleIdentifier"];
				if (ident)
				...
				if ([firslCol isEqualToString:@"Bundle Identifier"])
					...
					self.name = [self.app objectForKey:@"CFBundleName"];
				else if ([firslCol isEqualToString:@"Bundle Display Name"])
					self.name = [self.app objectForKey:@"CFBundleDisplayName"];
			}
...
{
	return (PSProc *)[self.procs objectAtIndex:idx];
}
...
- (PSProc *)procForPid:(pid_t)pid
{
	...
	return idx == NSNotFound ? nil : (PSProc *)[self.procs objectAtIndex:idx];
}
